...
def get_profile_page(profile_id):
    ...
    response = send_request(session=session, method="GET", link="https://www.natashaclub.com/" + profile_id)
    profile_page = get_parsed_page(response)

    return profile_page


def search_for_profiles(sex, looking_for, date_of_birth_start, date_of_birth_end, page=1, photos_only="off"):
    """Поиск профилей по заданным критериям.

    Keyword arguments:
    sex -- пол профиля
    looking_for -- поиск профилкй что имеют указанный пол
    date_of_birth_start -- минимальный возраст человека с кем профиль хочет познакомится
    date_of_birth_end -- максимальный возраст человека с кем профиль хочет познакомится
    page -- итерируемый параметр, указывает на страницу, где находится 10 профилей (default 1)

    """
    session = requests.Session()
    if photos_only == "on":
        search_link = f"https://www.natashaclub.com/search_result.php?p_per_page=10&photos_only=on&Sex={sex}" \
                      f"&LookingFor={looking_for}&DateOfBirth_start={date_of_birth_start}" \
                      f"&DateOfBirth_end={date_of_birth_end}&CityST=0&City=&&page={page}&LastOnline=7"
        request = send_request(session=session, method="POST", link=search_link)
    else:
        search_link = f"https://www.natashaclub.com/search_result.php?p_per_page=10&Sex={sex}" \
                      f"&LookingFor={looking_for}&DateOfBirth_start={date_of_birth_start}" \
                      f"&DateOfBirth_end={date_of_birth_end}&CityST=0&City=&&page={page}&LastOnline=7"
        request = send_request(session=session, method="POST", link=search_link)
...
def get_id_profiles(searched_profiles_page):
    """Формирование списка ID профилей, вытянутых со страницы (поиск профилей по заданым критериям).

    Keyword arguments:
    searched_profiles_page -- страница профилей, отобранных по заданым киртериям
    """
    # Массив для внесения всех ссылок, что имеют target = _blank
    profiles_id = []
    # Перебираем все ссылки и оставляем только те, которые ведут на профиль зарегестрированого аккаунта
    """
    Пример собранных ссылок: /flowers.html, https://www.1st-international.com/html/check/ladycheckorder.html,
    # Fergt88.html, Fergt88.html, vkiss.php?sendto=1001615983, BRITO1985.html, BRITO1985.html

    Нам нужны ссылки vkiss.php?sendto=1001615983, в них есть ID профиля, его и будем вытягивать
    """
    for link in searched_profiles_page.find_all('a', target="_blank"):
        # Проверяем первые 5 символов, если совпадают с vkiss значит нужная ссылка
        if link.get("href")[:5] == "vkiss":
            # Вытягиваем ID профиля, он начинается с 17 символа в вытянутой ссылке
...
def collect_info_from_profile(profile_id):
    ...
    # Имя
    name = profile_page.find("td", class_="ContentHeaders").find("h1").text.split(":")[0]
    if not name:
        name = "Not specified"
    ...
    # Собираем информацию с таблицы PERSONAL DETAILS и DETAILS OF THE PERSON YOU ARE LOOKING FOR
    table_personal_details = profile_page.find_all("table", class_="profile_table")
    personal_details = {}
    for row in table_personal_details[0].find_all("tr")[2].find_all("tr"):
        ...
        if value and key:
            ...
            if len(value) == 0:
                value = "Not specified"
            # Проверка есть ли 1 в value означает что пользователь указал свой рост (все росты начинаются с цифры 1)
    ...
    rate = float(rating_table[0].find_all("table", class_="text2")[0].find("td").text.split(" ")[1])
    votes = int(rating_table[0].find_all("table", class_="text2")[1].find("td").text.split(" ")[1])

    # Формируем словарь, который имеет в себе все собранные данные
    collected_data = {"Name": name, "Nickname": nickname, "Age": age, "Sex": gender, "Zodiac": zodiac,
                      "Country": country, "City": city, "Languages": languages, "Occupation": occupation,
                      "Children": children, "Last online": str(last_online),
                      "Credits to open letter": cost_credits}
    collected_data.update(personal_details)
    collected_data.update({"Rate": rate, "Votes": votes, "Description": description,
