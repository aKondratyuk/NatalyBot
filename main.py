...
from file_operations import create_custom_message, read_excel_file, \
    get_list_of_all_files_in_directory, get_sheet_values
from messaging import message
from scraping import send_request, search_for_profiles, get_id_profiles, collect_info_from_profile
from verification import limit_out, forbidden_profile, profile_in_inbox, first_letter_sent, profile_country, \
    profile_deleted, not_in_age_bounds


def check_for_file(session, my_age, profile_id, idx):
    try:
        if profile_in_inbox(session, profile_id)[0]:
            print(f"{idx}. Неудалось отправить сообщение {profile_id}", datetime.now(), "INBOX")
            return "INBOX"
        elif limit_out(session, profile_id):
            print(f"{idx}. Неудалось отправить сообщение {profile_id}", datetime.now(), "LIMIT OUT")
            return "LIMIT OUT"
        elif profile_deleted(profile_id):
            print(f"{idx}. Неудалось отправить сообщение {profile_id}", datetime.now(), "PROFILE DELETED")
            return "PROFILE DELETED"
        elif forbidden_profile(profile_id):
            print(f"{idx}. Неудалось отправить сообщение {profile_id}", datetime.now(), "FORBIDDEN")
            return "FORBIDDEN"
        elif not_in_age_bounds(my_age, profile_id):
            print(f"{idx}. Неудалось отправить сообщение {profile_id}", datetime.now(), "AGE IS YOUNGER")
            return "AGE IS YOUNGER"
        elif first_letter_sent(session, profile_id):
            print(f"{idx}. Неудалось отправить сообщение {profile_id}", datetime.now(), "FIRST LETTER SENT")
            return "FIRST LETTER SENT"
        elif profile_country(profile_id):
            print(f"{idx}. Неудалось отправить сообщение {profile_id}", datetime.now(), "INVALID COUNTRY")
...
def login(profile_login, password):
    ...
    """
    # Создаем сессию для отправщика (не имея фиксированую сессию залогинено аккаунта не получится отправить сообщение)
    ...
    request = send_request(session=session, method="POST",
                           link="https://www.natashaclub.com/member.php", data=data)

    # Если успешно вошли на сайт, то должен создаться logtrackID для залогиненого аккаунта
    if "logtrackID" in request.text:
        ...
        # Функция возвращает False если логин или пароль оказались неправильными
        return False
...
def tabulated_table(sheet):
    ...
    for i, profile_id in enumerate(data["Login"]):
        try:
            ...
            print(f"Профиль номер {i + 1} с логином {profile_id} не существует")
        row = [i + 1, profile_id, data["Password"][i + 1], profile["Name"],
               profile["Nickname"], profile["Age"], data["Path to text file"][i + 1]]
        new_data.append(row)
    header = ["Номер", "Логин", "Пароль", "Имя", "Никнейм", "Возраст", "Имя инвайта"]
    # print(tabulate(new_data, headers=header, tablefmt="grid"))
    from pprint import pprint
    pprint(data)
    print(data["Login"][1])
    return new_data  # old return: data


if __name__ == "__main__":
    # print(create_custom_message("1001706595","1000091902",""))
    # pass
    sheet = "Empty"
    table = "Empty"
    while True:
            ...
            if sheet == "Empty":
                ...
                while True:
                    ...
                    if looking_for == "male":
                        ...
                        print("Ищу - женщины")
                    print("Возраст от - (ставится возраст профиля автоматически)")
                    print(f"Возраст до - {date_of_birth_end}")
                    print(f"Рассылка начинается со страницы - {page}")
                    print(f"Будет отправлено сообщений - {messages_need_to_be_sent}")
                    if photos_only == "off":
                    ...
                    if choice == "1":
                        ...
                        if choice == "1":
                            ...
                            print(f"Изменено на: Возраст до - {date_of_birth_end}")
                        elif choice == "3":
                            messages_need_to_be_sent = int(input("Укажите количетсво сообщний: "))
                            print(f"Изменено на: Будет отправлено сообщений - {messages_need_to_be_sent}")
                        elif choice == "4":
                            page = int(input("Укажите номер страницы с какой начать: "))
                            print(f"Изменено на: рассылка начинается со страницы - {page}")
                        elif choice == "5":
                            photos_only = "on"
                        ...
                        data = tabulated_table(sheet)
                        for i in range(1, len(data["Login"])):
                            profile_login, password = str(data["Login"][i]), data["Password"][i]
                            values = login(profile_login, password)
                            if values:
                                session, my_profile_id = values
            ...
            if len(files) > 0:
                ...
                if int(i) <= len(files):
                    ...
                    if int(i) <= len(sheets_payers):
                        ...
                        messages_need_to_be_sent = 30
                        # print(f"Будет отправлено {messages_need_to_be_sent} сообщений\n")
                        # print("1. Изменить количество отправляемых сообщений")
                        # print("2. Выбрать аккаунты, которые будут рассылать")
                        # print("3. Начать рассылку всех аккаунтов")
                        # print("0. Вернуться к главному меню")
                        # choice = input("\nВыберите пункт меню: \n")
                        while True:
                            ...
                            choice = input("\nВыберите пункт меню: \n")
                            if choice == "1":
                                messages_need_to_be_sent = int(input("Укажите количетсво сообщений: "))
                            elif choice == "2":
                                data = tabulated_table(sheet)
                                choice = input("\nНапишите через запятую номера аккаунтов: \n")
                                numbers = [int(num) for num in choice.split(",")]
                                    ...
                                    if values:
                                        ...
                                        for profile_id in payers:
                                            if STOP:
                                                break
                                            try:
                                                check_response = check_for_file(session, my_age, profile_id, idx)
                                                idx += 1
                                                if check_response:
                                                    if check_response == "LIMIT OUT":
                                                        STOP = True
                                            except Exception as ex:
                                                print("Ошибка тут", profile_id)
                                            else:
                                                invite_path = "invite messages/" + data["Path to text file"][i]
                                                message_text = create_custom_message(my_profile_id, profile_id,
                                                                                     invite_path)
                                                message(session, profile_id, message_text)
                                                print(
